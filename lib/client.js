window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* 按查看时刻（北京时间）解析峰谷价格档。
		* 2026-08-23（周日）00:00 起，周末（周六/周日）全天统一按低谷/空闲时段计费。
		...
		*/
		function priceFor(model, nowMs) {
			...
			const isWeekendOffpeak = bj.getTime() >= Date.UTC(2026, 7, 23, 0, 0, 0) && (day === 0 || day === 6);
			const hour = bj.getUTCHours();
			const peak = !isWeekendOffpeak && ((hour >= 9 && hour < 12) || (hour >= 14 && hour < 18));
