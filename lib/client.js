window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* https://api-docs.deepseek.com/zh-cn/quick_start/pricing（2026-09-18）。
		* 峰谷定价：高峰时段为北京时间周一至周五 9:00-12:00、14:00-18:00；
		* 其余时段（含周六、周日全天）为空闲，空闲 = 高峰一半。
		*   deepseek-flash      空闲 0.02 / 1.0 / 4.0   高峰 0.04 / 2.0 / 8.0
		*   deepseek-v4-pro     空闲 0.15 / 4.5 / 13.5  高峰 0.30 / 9.0 / 27.0
		* 旧 id（deepseek-v4-flash、deepseek-v4-flash-vision-exp）已下线，但请求仍
		* 由 DeepSeek-V4.1-Flash 提供服务，按 Flash 价计费，故与 deepseek-flash 同价。
		...
		*/
		...
		/**
		* 将模型 id 归一到 MODELS 的键：精确命中优先；带版本后缀的 id
		* （如 deepseek-v4-pro-0813）归到所属系列，避免被误按 flash 兜底价计费。
		*/
		...
		/**
		* 按查看时刻（北京时间）解析峰谷价格档。
		* 高峰：周一至周五 9:00-12:00、14:00-18:00；其余（含周末全天）为空闲。
		...
		*/
		function priceFor(model, nowMs) {
			...
			const peak = day !== 0 && day !== 6 && ((hour >= 9 && hour < 12) || (hour >= 14 && hour < 18));
		...
		*/
		function SessionStatsPanel(props) {
			...
			const api = props.api;
			const listState = react.useSyncExternalStore(
				(onStoreChange) => sessions && sessions.list && typeof sessions.list.subscribe === "function" ? sessions.list.subscribe(onStoreChange) : () => {},
				() => sessions && sessions.list && typeof sessions.list.getSnapshot === "function" ? sessions.list.getSnapshot() : undefined
			);
			const summary = listState === void 0 || listState.current === void 0 ? void 0 : listState.byId[listState.current];
