 ...
 * Effective 2026-08-23 00:00 (Beijing time), weekends (Saturday & Sunday) are entirely off-peak.
 * @param {Date} bjDate
 */
export function isPeakTime(bjDate) {
	const day = bjDate.getUTCDay();
	const isWeekendRuleActive = bjDate.getTime() >= Date.UTC(2026, 7, 23, 0, 0, 0);
	if (isWeekendRuleActive && (day === 0 || day === 6)) {
		return false;
	}
